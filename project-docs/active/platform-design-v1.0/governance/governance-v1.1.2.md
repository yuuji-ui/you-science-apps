...
# Governance Specification Ver.1.1.2
...
| 対応Requirements | YSA-REQ-001 Ver.1.2.3 |
| 対応Architecture | YSA-ARCH-001 Ver.1.1.5 |
| 作成日 | 2026-08-03 |
| 文書責任者 | Project Owner |
| 設計担当 | ChatGPT |
| 第三者レビュー | Claude（未実施） |
...
通常の表示順・Label変更はPortal Maintainerが実施し、別担当者が差分を確認する。誤字修正等は事後確認を許容するが、featured、非表示、group再編は事前Reviewを必要とする。
...
Level判定の客観基準はArchitecture Specificationが参照する`ADR-0011 教材Level客観判定`を正本とする。
...
```text
YSA-REQ-001 Requirements Specification Ver.1.2.3
YSA-ARCH-001 Architecture Specification Ver.1.1.5
```
