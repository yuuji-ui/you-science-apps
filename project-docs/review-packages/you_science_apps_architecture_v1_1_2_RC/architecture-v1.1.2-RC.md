...
| 対応Governance | YSA-GOV-001 Ver.1.1 Draft |
| 作成日 | 2026-08-03 |
| 文書責任者 | Project Owner |
| 設計担当 | ChatGPT |
| 第三者レビュー | Claude（未実施） |
| 参照検証状態 | Requirements参照自動検証済み |
| 改訂区分 | PATCH / Release Candidate化 |
...
共通機能は複数教材で必要性を確認してからPlatform Coreへ採用する。
原則として2〜3教材以上で再利用性を確認する。
...
- `shared`：公開Platform Coreを参照
- `vendored`：特定Core Versionを教材内へ同梱
- `standalone`：Runtime共有依存なし

ManifestへDependency ModeとCore Versionを記録する。

対応要求：
REQ-MAN-005、REQ-MNT-003、REQ-OFF-005、REQ-MAN-006
...
対応要求：
REQ-MAN-001、REQ-MAN-002、REQ-MAN-003、REQ-MAN-005、REQ-MAN-006、
...
対応要求：
REQ-CAT-003、REQ-CAT-007、REQ-CAT-008、REQ-CAT-009、REQ-CAT-010
...
Core 1.x、2.x、Standaloneを一定期間併存可能とする。
...
1. Requirements Ver.1.2.2 RC参照がERROR 0件
