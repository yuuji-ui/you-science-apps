...
def to_catalog_app(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "portalGroups": education["portalGroups"],
        "audiences": education["audiences"],
        "categories": education["categories"],
        "tags": education["tags"],
        "learningDifficulty": education["learningDifficulty"],
...
def apply_override(app: dict[str, Any], override: dict[str, Any]) -> None:
    ...
    for key, value in override.items():
        if key not in allowed:
            raise CatalogError(f"Override for {app['appId']} contains forbidden field: {key}")
        app[key] = value
...
def validate_groups(groups: list[dict[str, Any]], app_ids: set[str]) -> None:
    ...
    for group in groups:
        ...
        if group_id in seen:
            raise CatalogError(f"Duplicate groupId: {group_id}")
        seen.add(group_id)
        missing = sorted(set(group["appIds"]) - app_ids)
        if missing:
            raise CatalogError(
                f"Group {group_id} references unknown appId(s): {', '.join(missing)}"
            )
...
def generate_catalog(
    ...
    for path in manifest_paths:
        ...
        required = [
            "appId", "title", "version", "updatedOn", "status", "level",
            "education", "distribution", "privacy", "license"
        ]
        ...
        if app_id in source_paths:
            raise CatalogError(f"Duplicate appId {app_id}: {source_paths[app_id]} and {path}")
        source_paths[app_id] = str(path)

        if manifest["status"] in PUBLISHABLE_STATUSES:
            apps.append(to_catalog_app(manifest))

    overrides = load_json(override_path).get("apps", {})
    known_ids = {app["appId"] for app in apps}
    unknown = sorted(set(overrides) - known_ids)
    if unknown:
        raise CatalogError(
            "Override references unknown or unpublished appId(s): " + ", ".join(unknown)
        )

    for app in apps:
        if app["appId"] in overrides:
            apply_override(app, overrides[app["appId"]])
    ...
    groups = load_json(groups_path).get("groups", [])
    validate_groups(groups, {app["appId"] for app in apps})
