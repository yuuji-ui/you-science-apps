...
def run(command, cwd):
    ...
    if proc.returncode != 0:
        raise CIError("Command failed: " + " ".join(command) + "\n" + proc.stdout + "\n" + proc.stderr)
    return result
...
def main():
    ...
    try:
        ...
        manifests = sorted((repo/"site/apps").rglob("app.manifest.json")) if (repo/"site/apps").exists() else []
        if manifests:
            ...
            report["steps"].append({
                "result":"PASS",
                "message":"No app manifests under site/apps; validate and deploy committed catalog."
            })
        ...
        forbidden = ["google-analytics.com","googletagmanager.com","gtag(","doubleclick.net","adsbygoogle"]
        found = [x for x in forbidden if x in index]
        if found:
            raise CIError("Forbidden analytics/advertising markers found: " + ", ".join(found))
        report["steps"].append({"result":"PASS","message":"Static privacy checks passed."})
